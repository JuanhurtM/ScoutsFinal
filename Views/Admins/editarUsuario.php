<?php
// Establecer tiempo de expiración 

session_start();
if ($_SESSION['rol'] == 1 || $_SESSION['rol'] == 2 || $_SESSION['rol'] == 3){

?>
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8">
    <title>Scouts Centinelas 113 | Editar Usuario</title>
    <link rel="shortcut icon" href="../../Assets/Img/LogoScout2.png">
    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/bootstrap/5.1.3/css/bootstrap.min.css">
    <link rel="stylesheet" href="/Css/form.css">
    <link rel="stylesheet" href="/Css/login.css">
    <!-- SweetAlert -->
    <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
  </head>
  <body>
    <div class="container-fluid">
        <div class="row flex-nowrap">
            <div class="col-auto col-md-3 col-xl-2 px-sm-2 px-0 bg-light">
                <div class="d-flex flex-column align-items-center align-items-sm-start px-3 pt-2 text-secondary min-vh-100">
                  <a href="#" class="d-flex align-items-center pb-3 mb-md-0 me-md-auto text-secondary text-decoration-none">
                    <img src="../../Assets/Img/LogoScout2.png" alt="Scouts" style="height: 5rem;">
                  </a>
                    <ul class="nav nav-pills flex-column mb-sm-auto mb-0 align-items-center align-items-sm-start mt-5" id="menu">
                        <li class="mt-2">
                            <a href="inicio.php" class="nav-link align-middle px-0 fs-5 fw-bold " style="color: #c50d0d;" >Inscripciones</a>
                        </li>
                        <li class="mt-2">
                            <a href="gestionPersonal.php" class="nav-link text-white text-center align-middle px-0 fs-5 fw-bold rounded-pill" style="background-color: #c50d0d; width: 12rem;">Personal</span></a>
                        </li>
                        <li class="mt-2">
                            <a href="cartera.php" class="nav-link px-0 align-middle fs-5 fw-bold " style="color: #c50d0d;">Cartera</span></a>
                        </li>
                    </ul>
                  <hr>
                  <div class="pb-4">
                    <a onclick="return csesion()" href="../../Controller/login/Exit.controller.php" class=" btn fw-bold fs-5 text-light text-center mt-5 mb-5" style="background-color: #c50d0d;">Cerrar Sesion</a>
                  </div>
                </div>
            </div>
            <div class="col py-3" >
                <div class="mt-5">  <!-- tratar con este contener -->
                    <div class="container">
                        <div class="row">
                            <div class="col">
                                <h1 class="fw-bold mb-5" style="color:#c50d0d;">Editar Usuario</h1>
                                <?php

                                    $NoIdentidad = $_GET['id'];
                                    include("../../Models/Conexion.php");
                                
                                    $sql = "SELECT * FROM usuarios WHERE NoIdentidad = '$NoIdentidad'";
                                    $query = mysqli_query($mysqli, $sql);

                                    $datos =  mysqli_fetch_array($query);

                                    $NoIdentidad2 = $datos['NoIdentidad'];
                                    $Nombre = $datos['Nombre'];
                                    $Correo = $datos['CorreoElectronico'];
                                    $Rol = $datos['Rol'];

                                ?>
                                <form class="mt-4" method="post" action="../../Controller/editar/editarUsuario.controller.php"  >
                                    <div class="row g-3 mb-6">
                                        <div class="col-6">
                                            <label for="Correo" class="form-label mb-2 fw-bold">Numero de Identidad *</label>
                                            <input type="text" value="<?php echo $NoIdentidad2; ?>" name="NoIdentidad" class="form-control" placeholder="Numero de Identidad" aria-label="NumeroIdentidad">
                                        </div>
                                        <div class="col-6">
                                            <label for="Nombre" class="form-label mb-2 fw-bold">Nombre Completo *</label>
                                            <input type="text" value="<?php echo $Nombre; ?>" name="nombre" class="form-control" placeholder="Nombre Completo" aria-label="Nombre">
                                        </div>
                                        <div class="col-6">
                                            <label for="Correo" class="form-label mb-2 fw-bold">Contraseña *</label>
                                            <input type="password" name="contraseña" class="form-control" placeholder="contraseña" aria-label="Contraseña">
                                        </div>
                                        <div class="col-6">
                                            <label for="Correo" class="form-label mb-2 fw-bold">Correo Electronico *</label>
                                            <input type="email" value="<?php echo $Correo;?>" name="correo" class="form-control" placeholder="Correo Electronico" aria-label="Ocupacion">
                                        </div>
                                        <div class="col-6">
                                            <label for="Correo" class="form-label mb-2 fw-bold">Rol *</label>
                                            <select name="rol" id="Rol" class="form-select" required>
                                                <option value=""disabled selected>
                                                    <?php 
                                                        if($Rol == 2){
                                                            echo "Jefe de Grupo";
                                                       } else if ($Rol == 3){
                                                            echo "Jefe de Rama";
                                                       }else{
                                                           echo "Tesorero";
                                                       }
                                                    ?>
                                                </option>
                                                <option value="2">Jefe de grupo</option>
                                                <option value="3">Jefe de rama</option>
                                                <option value="5">Tesorero</option>
                                            </select>
                                        </div>
                                    </div>
                                    
                                    <button type="submit" class="btn fw-bold text-light text-center mt-5 mb-3" style="background-color: #c50d0d;">Guardar</button>
                    
                                </form>
                                <a href="gestionPersonal.php" class="btn btn-success text-light fw-bold">Volver</a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- Bootstrap JavaScript -->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/bootstrap/5.1.3/js/bootstrap.bundle.min.js"></script>
  </body>
</html>
<?php
}else{
    header("Location: ../../index.php");
}

?>